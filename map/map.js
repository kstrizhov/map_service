...
leafletMap.setView([ 55.7422, 37.5719 ], 8);
...
var viewVSP = L.easyButton({
	states : [ {
		stateName : 'view',
		icon : '<img src="view.png" height=65%/>',
		title : 'view VSP markers',
		onClick : function(control) {
			getPoints();
			control.state('hide');
		}
	}, {
		icon : '<img src="hide.png" height=65%/>',
		stateName : 'hide',
		onClick : function(control) {
			leafletMap.removeLayer(markers);
			control.state('view');
		},
		title : 'hide VSP markers'
	} ]
});
viewVSP.addTo(leafletMap);
...
function getRouteGeometryString() {
	...
	var loc1LatLng = String(loc1.getLatLng().lat) + ','
			+ String(loc1.getLatLng().lng);
	var loc2LatLng = String(loc2.getLatLng().lat) + ','
			+ String(loc2.getLatLng().lng);
	var params = 'loc=' + loc1LatLng + '&loc=' + loc2LatLng
			+ '&instructions=true';
...
function decodeRouteGeometryString(str, precision) {
	var index = 0, lat = 0, lng = 0, coordinates = [], shift = 0, result = 0, byte = null, latitude_change, longitude_change, factor = Math
			.pow(10, precision || 5);
...
function drawRoute() {
	...
	polyline = new L.polyline(routeCoordinates, {
		color : 'red'
	});
